...
struct SystemDetailView: View {
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    // Recommendations (if issues found or unknown)
                    if status == "Needs Attention" {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            HStack(alignment: .top, spacing: 12) {
                                ...
                                Text("Get a professional inspection.")
                                    .font(.system(size: FontSize.bodyLarge))
                                    .foregroundColor(.textPrimary)
                            }
                        }
    ...
    private var statusColor: Color {
        ...
        return status == "Good" ? .statusSafe : .statusCaution
    }
    
    private var statusBackgroundColor: Color {
        if status == "Unknown" { return Color.gray.opacity(0.15) }
    ...
    private var overviewText: String {
        if status == "Unknown" {
            return Self.unknownOverviewText
        }
        switch section {
        case "Engine":
            return status == "Good"
        ...
        case "Electrical":
            return status == "Good"
                ? "The electrical system is functioning properly. Battery voltage is healthy and all sensors are responding correctly."
                : "Battery voltage is outside the normal range. The battery or alternator may need attention."
        default:
